...
//这里按着给的注解修改参数即可
        ...
        //手动配置数据源
        String url="jdbc:mysql://localhost/alvin_mall?useUnicode=true&useSSL=false&characterEncoding=utf8&allowPublicKeyRetrieval=true";   //注意修改数据库名
        String name="root";
        String password="";

        //数据库表的设置
        List<String> listTable = Arrays.asList("ums_user");  //设置需要自动代码生成的表名
        List<String> listTableSuffix = Arrays.asList("_b");    //设置 过滤 表的后缀
        List<String> listTablePrefix = Arrays.asList("t_","c_"); //设置 过滤 表的后缀

        //基本信息
        String author = "Alvin";    //作者
        String parent = "org";   //父包名
        String module = "wrj";   //模块包名
        ...
        FastAutoGenerator.create(url, name, password)
                ...
                .globalConfig(builder -> {
                    builder.author(author) // 设置作者名
                            .outputDir(System.getProperty("user.dir") + "/mp/src/main/java")   //设置输出路径：项目的 java 目录下【System.getProperty("user.dir")意思是获取到项目所在的绝对路径】
                ...
                .packageConfig(builder -> {
                    builder.parent(parent) // 设置父包名
                            ...
                            .pathInfo(Collections.singletonMap(OutputFile.mapperXml, System.getProperty("user.dir")+"/mp/src/main/resources/mapper"));    //配置 mapper.xml 路径信息：项目的 resources 目录下
                })
                //4、策略配置
